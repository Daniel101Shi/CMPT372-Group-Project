...
export function CoursesContextProvider({ children }: { children: React.ReactNode }) {
    ...
    let [courses, courses_setter] = useState<{department: string, courses: {c_number: string, c_title: string}[]}[]>([]);
    ...
    async function fetch_dept_courses(deptartment: string, year: string, semester: string): Promise<{department: string, courses: {c_number: string, c_title: string}[]}> {
        ...
        const res_courses = json.map(x => ({c_number: x.value, c_title: x.title}))
        return {
            department: deptartment,
            courses: res_courses
        }
    ...
    async function fetch_courses(depts: string[], year: string, semester: string): Promise<{department: string, courses: {c_number: string, c_title: string}[]}[]> {
